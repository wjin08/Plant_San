...
int main(int argc, char *argv[])
{
	...
	struct sockaddr_rc addr = { 0 };
  	char dest[18] = "98:DA:60:0D:A1:FB";	//iot12
	char msg[BUF_SIZE];
    ...
    // 라즈베리파이에 내장된 블루투스 모듈과 통신하기 위해 소켓을 하나더 생성
	if(dev_fd.btfd == -1){
		perror("socket()");
		exit(1);
	}
	...
	str2ba(dest, &addr.rc_bdaddr); //MAC주소로 블루투스 모듈 연결
    ...
    char ary[100]="hello";
    write(dev_fd.sockfd,ary,strlen(ary));
