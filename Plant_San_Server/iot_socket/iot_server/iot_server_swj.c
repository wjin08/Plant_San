...
int main(int argc, char *argv[])
{
    ...
    if (!(mysql_real_connect(conn, host, user, pass, dbname, 0, NULL, 0)))
    {
		...
		char id[ID_SIZE]; // id의 최대 길이는 10바이트
		char pw[ID_SIZE]; // pw의 최대 길이는 10바이트 

		CLIENT_INFO * client_info = (CLIENT_INFO *)calloc(sizeof(CLIENT_INFO),MAX_CLNT);
        // client_info 구조체 변수를 동적할당을 해서 메모리 사용 (크기는 -> MAX_CLNT)
		...
		do {
            ...
            // idpasswd.txt에는 (id passwd] 이런식으로 저장되어 있음. 

            // fscanf함수 설명
            // str_len에는 현재 행에서 읽어온 데이터의 개수가 저장됨.
            // 예를 들어서 위에서는 id,pw를 읽어 왔으니 2가 들어가고,
            // 하나가 없으면 1,
            // 아무것도행에 없다면 0
            // 파일의 끝이라면 -1을 리턴함
